...
class CifDetTriplet:
    meta: headmeta.CifDetTriplet
    rescaler: AnnRescalerDet = None
    v_threshold: int = 0
    bmin: float = 1.0  #: in pixels
    visualizer: CifDetVisualizer = None  # TODO change visualiser

    side_length: ClassVar[int] = 5
    padding: ClassVar[int] = 10

    def __call__(self, image, anns, meta):
        return CifDetTripletGenerator(self)(image, anns, meta)


class CifDetTripletGenerator:
    def __init__(self, config: CifDetTriplet):
        self.config = config
    ...
    def fill(self, detections):
        for category_id, bbox in detections:
            xy = bbox[:2] + 0.5 * bbox[2:]  # bbox center point (top left corner + half width height)
            wh = bbox[2:]
            self.fill_detection(category_id - 1, xy, wh)

    def fill_detection(self, f, xy, wh):
        # mapping xy coordinate representing bbox center to ij coordinate using scalar offset, padding, side length
